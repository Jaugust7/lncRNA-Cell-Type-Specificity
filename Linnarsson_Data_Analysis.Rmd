...
bar.plot + geom_bar(aes(x=rownames(level_2_barplot), y=tmp$Number_of_Clusters, fill=rownames(level_2_barplot)), colour="black", width=0.5, stat = "identity") + scale_fill_brewer(palette = "Set1", guide = guide_legend(title = "Cluster #")) + xlab("Cluster #") + ylab("Number of cellular subtypes") + theme_bw() + theme(axis.text.x = element_blank())

```

```{r Level2class_complete_split, tidy=TRUE}
...
t.level_2_barplot<-t(level_2_barplot)
t.level_2_barplot<-as.data.frame(t.level_2_barplot)
delta_median_by_cluster$Number_of_Clusters<- as.vector(t.level_2_barplot$Number_of_Clusters)

p<-ggplot(delta_median_by_cluster, aes(x=delta_median_mean_cpc, y=Number_of_Clusters))

scatter.plot<-p + geom_point(aes(color=rownames(delta_median_by_cluster))) + geom_smooth(aes(alpha=0.1),method = "lm", se = T, color="black", alpha=0.2) + scale_color_brewer(palette = "Set1") + guides(fill="none") + theme_bw() + theme(legend.position="none")
