...
tmp<-subset(dat, fData(dat)$ccdsid %in% "lincRNA")

lincRNA.dat.tSNE<-Rtsne(t(round(vstExprs(tmp))),theta=0.1, check_duplicates = F)  ## perplexity is too large???
...
lincRNA_level1class_tSNE<-q + geom_point(aes(x=tSNE1_pos,y=tSNE2_pos,color=level1class)) + theme_bw() + coord_equal(1) + scale_color_brewer(palette="Set1")
...
sex_blind_list<-as.vector(rownames(exprs(sex_blind.dat)[c(-1, -5),]))
