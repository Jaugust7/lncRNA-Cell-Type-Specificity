...
delta_BCV_by_cluster<-lapply(c(1:9), function(x){
  median(subset(mRNA_bcv, mRNA_bcv$cluster_num==x)$BCV) - median(subset(linc_bcv,linc_bcv$cluster_num==x)$BCV)
  })

names(delta_BCV_by_cluster)<-c("Cluster_1", "Cluster_2", "Cluster_3", "Cluster_4", "Cluster_5", "Cluster_6", "Cluster_7", "Cluster_8", "Cluster_9")

as.matrix(delta_BCV_by_cluster)
```
...
p + geom_point(aes(x=log10(BCV), y=values(unique(level2_bcv)$level2class)))
