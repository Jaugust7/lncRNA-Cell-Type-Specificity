...
Cluster.split<-lapply(c(1:length(Cluster.split)), function(x){ detectGenes(Cluster.split[[x]], min_expr = 0.01)})
...
barplot(as.matrix(level_2_barplot))
...
bar.plot + geom_bar(aes(x=rownames(tmp), y=tmp$Number_of_Clusters, fill=rownames(tmp)), colour="black", width=0.5, stat = "identity") + scale_fill_brewer(palette = "Set1") + xlab("Cluster") + ylab("Number of cellular subtypes") + guides(fill="none") + theme_bw()
 ...
 scatter.plot <- ggplot(group_means) + geom_point(aes(x=log10(mean_cpc), 
                                                             y=group_num, 
                                                             color=gene_type)) + 
